const { SBFParser: Parser } = require('@coremarine/septentrio-sbf')
...
module.exports = function (RED) {
  ...
  function SBFParser (config) {
    ...
    node.on('input', (msg, send, done) => {
      ...
      switch (command) {
        ...
        // MEMORY ------------------------------------
        case 'setMemory':
          if (isBoolean(payload)) { parser.memory = payload }
          send(null)
          break
        case 'getMemory':
  ...
  // Register
  RED.nodes.registerType('cma-septentrio-parser', SBFParser)
}
