...
// `@coremarine/nmea-parser-nodered@N.x` wraps `@coremarine/nmea-parser@N.x`.
//
...
const LIBRARY = '@coremarine/nmea-parser'
const wrapper = read(join(here, '..', 'package.json'))
const library = read(join(here, '..', '..', 'nmea-parser', 'package.json'))
