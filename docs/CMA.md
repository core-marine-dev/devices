...
- Older draft allowed `timestamp: number | string` (ISO). Current draft: **epoch ms only**.
- Older draft had `protocol.version` optional and `Protocol & Record<string, any>`; current
  draft requires `version` and is closed. Decide before rollout.
- `thelmabiotel-tblive` (the only conforming parser) adds top-level `mode` and `firmware` keys
  not in the draft — decide: allow extra top-level keys, or push them into `metadata`?
- `bool` vs `boolean` in `Type` (old draft said `bool`, current draft says `boolean`).
